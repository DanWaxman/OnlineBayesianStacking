"""
Modified version of mnist_clf.py from the official BONG GitHub repository
https://github.com/petergchang/bong/blob/main/bong/experiments/mnist_clf.py
Copyright (c) 2022 Probabilistic Machine Learning, MIT License

It should be run with BONG installed from their GitHub repository.
"""
import os
...
def callback_fn(key, alg, state, x, y, em_function, X_cb, y_cb, num_samples=10):
    ...
    nll_pi_full = optax.softmax_cross_entropy_with_integer_labels(ypi_pred_logits, y_cb)
    ...
    return nll_pi, miscl_pi, nll, miscl, nll_lin, miscl_lin, nll_pi_full
        ...
        for n_sample in n_samples:
            for n_iter in n_iters:
                ...
                if params_to_use is not None:
                    best_hparams = params_to_use
                elif curr_hparam_path.exists():
...
def evaluate_agents(key, agent_queue, X_tr, Y_tr, em_function, eval_niter, result_path, X_te, Y_te):
    key, subkey = jr.split(key)
    result_dict = {}
    ...
    for agent_name, agent in agent_queue.items():
        ...
        for _ in tqdm.trange(eval_niter):
            shuffle_key = jr.PRNGKey(_)
            key1, key2, subkey = jr.split(subkey, 3)
            idx = jr.permutation(shuffle_key, jnp.arange(X_tr.shape[0]))
            X_tr, Y_tr = X_tr[idx], Y_tr[idx]
            print(np.mean(X_tr[0]), np.mean(X_tr[5]), np.mean(X_tr[14]), np.mean(X_tr[100]))
            eval_cb_fn = partial(callback_fn, em_function=em_function, X_cb=X_tr, y_cb=jax.numpy.argmax(Y_tr, axis=-1))
            _, (nll_pi, miscl_pi, nll, miscl, nll_lin, miscl_lin, nll_pi_full) = (
            ...
            nll_pi_fulls.append(nll_pi_full)
            runtimes.append(t1 - t0)
        nll_pis, miscl_pis, nlls, miscls, nll_lins, miscl_lins, nll_pi_fulls, runtimes = (
            _farray(nll_pis),
            _farray(miscl_pis),
            _farray(nlls),
            _farray(miscls),
        ...
        # print('aaa', nll_pi_fulls)
        nll_pi_fulls_dict[agent_name] = np.array(nll_pi_fulls)
        print(
        ...
        print("=====================================")
    return result_dict, nll_pi_fulls_dict


def save_results(result_dict, nll_pi_fulls, result_path, init_cov):
    ...
    for agent_name, agent_result in nll_pi_fulls.items():
        with open(Path(result_path, f"{agent_name}_nlls_{init_cov}.pkl"), "wb") as f:
            pickle.dump(agent_result, f)
    # Save runtime
    fig, ax = plt.subplots()
...
def main(args):
    ...
    agent_queue, key = tune_agents(
        ...
        hparam_path,
        args.tune_niter,
        params_to_use={'init_cov': np.sqrt(args.init_var)},
    )

    # Evaluate agents
    result_path = Path(mnist_path, args.model)
    result_path.mkdir(parents=True, exist_ok=True)
    result_dict, nll_pi_fulls = evaluate_agents(
        key, agent_queue, X_tr, Y_tr, em_function, args.eval_niter, result_path, X_te, Y_te
    )

    print(nll_pi_fulls)
    # Save results
    save_results(result_dict, nll_pi_fulls, result_path, args.init_var)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["dlrg-l-ef-bong"],
        choices=BONG_DICT.keys(),
    )
    parser.add_argument(
        "--dlr_ranks",
        type=int,
    ...
    parser.add_argument("--features", type=int, nargs="+", default=[64, 32, 10])
